...
class WPMI_Admin
{
    ...
    public function admin_init()
    {
        ...
        add_settings_field(
            'wpmi_comment_group', // ID
            __('Comment authors Groups', 'wpmi'), // Title
            array($this, 'comment_group_callback'), // Callback
            'wpmi_general_page', // Page
            'wpmi_general' // Section
        );
    }
    ...
    public function comment_group_callback()
    {
        $api_key = self::get_option('api_key');
        if( isset($api_key) && !empty($api_key) ) {
	        $all_groups = $this->group_api->get();
	        if ($all_groups) {
		        echo '<select name="wpmi_options[comment_groups][]" id="comment_groups" multiple="multiple" class="wpmi_select2">';
		        echo '<option value="" >'.__('Select groups', 'wpmi').'</option>';
		        foreach ($all_groups as $group) {
			        $selected = (in_array($group->id, self::get_option('comment_groups'))) ? ' selected="selected"' : '';
			        echo '<option value="'.$group->id.'" '.$selected.'>'.$group->name.' ( '.$group->total.' '.__('User', 'wpmi').')</option>';
		        }
		        echo '</select>';
	        }
    ...
    public static function get_option($option_name = 'all')
    {
        ...
        $comment_groups = (isset($options['comment_groups']) && !empty($options['comment_groups'])) ? $options['comment_groups'] : array();
